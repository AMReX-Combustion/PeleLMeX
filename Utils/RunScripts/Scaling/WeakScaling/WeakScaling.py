...
USAGE = """
    A script to setup weak scaling of PeleLMeX.
...
def setBatchScript(args,caseFolder,case,host):
    ...
    with open(args.batch_script) as fin, open(outfile, 'w') as fout:
        for line in fin:
            ...
            # Crusher/Frontier: 8 CPU+GPU pair / nodes
            if any(h in host[1] for h in ("Crusher","Frontier")):
                if "SBATCH -N" in line:
                    lineout = "#SBATCH -N {} \n".format(case)
                if "SBATCH -J" in line:
                    lineout = "#SBATCH -J {}_{:04d} \n".format(args.test_name,case)
                if "srun" in line:
                    lineout = "srun -N{} -n{} -c7 --gpus-per-node=8 --gpu-bind=closest ./{} {}".format(case,case*8,args.exec,args.input_file)
            fout.write(lineout)
...
def launchRun(args,host):
    ...
    if any(h in host[1] for h in ("Cori","Perlmutter","Crusher","Frontier")):
