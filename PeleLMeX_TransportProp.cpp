    ...
    if (m_incompressible == 0) {
      for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
        ...
        amrex::ParallelFor(
          ...
          [l_turb_ma, v_turb_ma,
           cp_ma] AMREX_GPU_DEVICE(int box_no, int i, int j, int k) noexcept {
            l_turb_ma[box_no](i, j, k) =
              cp_ma[box_no](i, j, k) * v_turb_ma[box_no](i, j, k);
          });
      }
      amrex::Gpu::streamSynchronize();
    }
...
PeleLM::getDiffusivity(
  ...
  const int beta_comp,
  const int ncomp,
  const int doZeroVisc,
  amrex::Vector<amrex::BCRec> const& bcrec,
