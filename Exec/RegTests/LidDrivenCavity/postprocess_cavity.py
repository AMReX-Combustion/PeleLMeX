...
handle plotfiles produced by the bundled
``input.2d-Re{100,400,1000}`` inputs (which use ``TanhStretchMap``
on both x and y).
...
1. Load the (one) plotfile via yt and pull u and v on a level-0
   covering grid.
2. Compute physical cell-center positions from the
   ``TanhStretchMap`` formula (read beta from CLI; default 2 2) -- yt
   reports Xi-space coordinates, which differ from physical space
   under mesh mapping.
3. Linearly interpolate u along the vertical centerline x = 0.5 and v
   along the horizontal centerline y = 0.5.
4. Overlay against Ghia tabulated points and save the comparison
   plots + a CSV.

Mesh-mapping options
====================

Same convention as ``../PipeFlow/postprocess_channel.py``: pass the
mapping kind and parameters via CLI flags.  Without flags the grid is
treated as uniform.  Currently supported: ``--map tanh`` with
``--beta beta_x beta_y``.
...
def tanh_offset_norm(eta: np.ndarray, beta: float) -> np.ndarray:
    """Normalized physical-space offset along an axis."""
    if abs(beta) < BETA_EPS:
        return eta
    return 0.5 * (1.0 + np.tanh(beta * (2.0 * eta - 1.0)) / math.tanh(beta))
...
def axis_phys_positions(prob_lo: float, prob_hi: float, N: int,
    ...
    eta = idx / N
    return prob_lo + L * tanh_offset_norm(eta, beta)
...
def main() -> int:
    ...
    ap.add_argument("--map", choices=("none", "tanh"), default="tanh",
                    help="mesh mapping kind (default: tanh, matching the "
                         "shipped input files)")
    ap.add_argument("--beta", type=float, nargs=2, default=(2.0, 2.0),
                    metavar=("BETAX", "BETAY"),
                    help="for tanh map: per-axis stretching strength "
                         "(default 2.0 2.0)")
    ...
    if args.map == "tanh":
        bx, by = args.beta
        x_phys = axis_phys_positions(plo[0], phi[0], Nx, bx, "cc")
        y_phys = axis_phys_positions(plo[1], phi[1], Ny, by, "cc")
    else:
    ...
    with csv_path.open("w") as fh:
        ...
        fh.write(f"# mapping = {args.map}, beta = "
                 f"{args.beta[0]} {args.beta[1]}\n")
