...
readHIT(HITData *a_data)
{
    ...
    amrex::Print(ofs).SetPrecision(17) << lambda0 << "," << a_data->urms0 << "," << tau << std::endl;
    ofs.close();
    ...
    a_data->d_xarray = (amrex::Real*) amrex::The_Arena()->alloc(nx*sizeof(amrex::Real));
    a_data->d_xdiff =  (amrex::Real*) amrex::The_Arena()->alloc(nx*sizeof(amrex::Real));
    a_data->d_uinput = (amrex::Real*) amrex::The_Arena()->alloc(nx*ny*nz*sizeof(amrex::Real));
    a_data->d_vinput = (amrex::Real*) amrex::The_Arena()->alloc(nx*ny*nz*sizeof(amrex::Real));
    a_data->d_winput = (amrex::Real*) amrex::The_Arena()->alloc(nx*ny*nz*sizeof(amrex::Real));
      ...
      char* argv[])
{
    Initialize(argc,argv);
    {
        ...
        RealBox rb_turb({D_DECL(xlo,xlo,xlo)},
                        {D_DECL(xhi,xhi,xhi)});
        int coord_turb(0);
        ...
        AMREX_PARALLEL_FOR_3D ( box_turb, i, j, k,
                                {
                                    fillVelFab(i,j,k,fab,geomdata,data);
                                });
