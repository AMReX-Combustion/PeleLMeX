...
PeleLM::readProbParm()
{
  ...
  amrex::Print(ofs).SetPrecision(17)
    << lambda0 << "," << prob_parm.urms0 << "," << tau << std::endl;
  ofs.close();
  ...
  PeleLM::prob_parm->d_xarray =
    (amrex::Real*)amrex::The_Arena()->alloc(nx * sizeof(amrex::Real));
  std::memcpy(
    &PeleLM::prob_parm->d_xarray, &prob_parm.d_xarray,
    sizeof(prob_parm.d_xarray));
  std::memcpy(
    &PeleLM::prob_parm->d_xdiff, &prob_parm.d_xdiff, sizeof(prob_parm.d_xdiff));
  std::memcpy(
    &PeleLM::prob_parm->d_uinput, &prob_parm.d_uinput,
    sizeof(prob_parm.d_uinput));
  std::memcpy(
    &PeleLM::prob_parm->d_vinput, &prob_parm.d_vinput,
    sizeof(prob_parm.d_vinput));
  std::memcpy(
    &PeleLM::prob_parm->d_winput, &prob_parm.d_winput,
    sizeof(prob_parm.d_winput));
