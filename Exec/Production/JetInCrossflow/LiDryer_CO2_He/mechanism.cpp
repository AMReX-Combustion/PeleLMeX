...
CKKFKR(
  ...
  amrex::Real q_r[])
{
  amrex::Real c[11]; // temporary storage
  amrex::Real PORT =
...
{
  amrex::GpuArray<amrex::Real, 144> Jac = {0.0};
  amrex::GpuArray<amrex::Real, 11> conc = {0.0};
  for (int n = 0; n < 11; n++) {
    conc[n] = 1.0 / 11.000000;
  }
...
SPARSITY_PREPROC_CSC(int* rowVals, int* colPtrs, const int* consP, int NCELLS)
{
  ...
  for (int nc = 0; nc < NCELLS; nc++) {
    int offset_row = nc * 12;
    int offset_col = nc * 12;
    for (int k = 0; k < 12; k++) {
      for (int l = 0; l < 12; l++) {
        if (Jac[12 * k + l] != 0.0) {
