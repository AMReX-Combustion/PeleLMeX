...
void PeleLM::WriteDebugPlotFile(const Vector<const MultiFab*>& a_MF,
	...
	if (m_write_hdf5_pltfile) {
		amrex::WriteMultiLevelPlotfileHDF5(
		    pltname, finest_level + 1, a_MF, names, Geom(), m_cur_time,
		    istep, refRatio());
	} else
#endif
	{
		amrex::WriteMultiLevelPlotfile(pltname, finest_level + 1, a_MF,
					       names, Geom(), m_cur_time, istep,
					       refRatio());
	}
}
...
void PeleLM::WritePlotFile() {
	...
	for (int ivar = 0; ivar < m_derivePlotVarCount; ivar++) {
		const PeleLMDeriveRec* rec =
		    derive_lst.get(m_derivePlotVars[ivar]);
	...
	for (int lev = 0; lev <= finest_level; ++lev) {
		mf_plt[lev].define(grids[lev], dmap[lev], ncomp, 0, MFInfo(),
				   Factory(lev));
	}
	...
	if (do_spray_particles && SprayParticleContainer::plot_spray_src) {
		...
		for (const auto& spray_fuel_name :
			...
			plt_VarsName.push_back("spray_" + spray_fuel_name +
					       "_src");
		}
	}
#endif
		...
		for (int ivar = 0; ivar < NUM_IONS; ++ivar) {
			for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
				...
				plt_VarsName.push_back(
				    "DriftFlux_" +
				    names[NUM_SPECIES - NUM_IONS + ivar] + "_" +
				    dir);
			}
		}
	}
	...
	for (int lev = 0; lev <= finest_level; ++lev) {
		int cnt = 0;
		if (m_incompressible) {
			MultiFab::Copy(mf_plt[lev], m_leveldata_new[lev]->state,
				       0, cnt, AMREX_SPACEDIM, 0);
			cnt += AMREX_SPACEDIM;
		} else {
			// Velocity and density
			MultiFab::Copy(mf_plt[lev], m_leveldata_new[lev]->state,
				       0, cnt, AMREX_SPACEDIM + 1, 0);
			cnt += AMREX_SPACEDIM + 1;
			// Species only if requested
			if (m_plotStateSpec) {
				MultiFab::Copy(mf_plt[lev],
					       m_leveldata_new[lev]->state,
					       FIRSTSPEC, cnt, NUM_SPECIES, 0);
				cnt += NUM_SPECIES;
			}
			MultiFab::Copy(mf_plt[lev], m_leveldata_new[lev]->state,
				       RHOH, cnt, 3, 0);
...
#ifdef PELELM_USE_SOOT
			MultiFab::Copy(mf_plt[lev], m_leveldata_new[lev]->state,
				       FIRSTSOOT, cnt, NUMSOOTVAR, 0);
			cnt += NUMSOOTVAR;
#endif
#ifdef PELELM_USE_RAD
			if (do_rad_solve) {
				MultiFab::Copy(mf_plt[lev], rad_model->G()[lev],
					       0, cnt, 1, 0);
				cnt += 1;
				MultiFab::Copy(mf_plt[lev],
					       rad_model->kappa()[lev], 0, cnt,
					       1, 0);
				cnt += 1;
				MultiFab::Copy(mf_plt[lev],
					       rad_model->emis()[lev], 0, cnt,
					       1, 0);
				cnt += 1;
			}
#endif
			if (m_has_divu) {
				MultiFab::Copy(mf_plt[lev],
					       m_leveldata_new[lev]->divu, 0,
					       cnt, 1, 0);
				cnt += 1;
			}
		}
		if (m_plot_grad_p) {
			MultiFab::Copy(mf_plt[lev], m_leveldata_new[lev]->gp, 0,
				       cnt, AMREX_SPACEDIM, 0);
			cnt += AMREX_SPACEDIM;
		}

		if (m_do_react && !m_skipInstantRR && m_plot_react) {
			MultiFab::Copy(mf_plt[lev], m_leveldatareact[lev]->I_R,
				       0, cnt, nCompIR(), 0);
			cnt += nCompIR();

			MultiFab::Copy(mf_plt[lev],
				       m_leveldatareact[lev]->functC, 0, cnt, 1,
				       0);
			cnt += 1;
			...
			if (m_plotHeatRelease) {
				...
				mf.reset(
				    new MultiFab(grids[lev], dmap[lev], 1, 0));
...
#ifdef AMREX_USE_EB
		MultiFab::Copy(mf_plt[lev], EBFactory(lev).getVolFrac(), 0, cnt,
			       1, 0);
		cnt += 1;
#endif
		...
		for (int ivar = 0; ivar < m_derivePlotVarCount; ivar++) {
			...
			MultiFab::Copy(mf_plt[lev], *mf, 0, cnt, mf->nComp(),
				       0);
			cnt += mf->nComp();
		}
#ifdef PELELM_USE_SPRAY
		if (SprayParticleContainer::NumDeriveVars() > 0) {
			const int num_spray_derive =
			    SprayParticleContainer::NumDeriveVars();
			mf_plt[lev].setVal(0., cnt, num_spray_derive);
			SprayPC->computeDerivedVars(mf_plt[lev], lev, cnt);
			if (lev < finest_level) {
				MultiFab tmp_plt(grids[lev], dmap[lev],
						 num_spray_derive, 0, MFInfo(),
						 Factory(lev));
				tmp_plt.setVal(0.);
				VirtPC->computeDerivedVars(tmp_plt, lev, 0);
				MultiFab::Add(mf_plt[lev], tmp_plt, 0, cnt,
					      num_spray_derive, 0);
			}
			cnt += num_spray_derive;
		}
		if (do_spray_particles &&
		    SprayParticleContainer::plot_spray_src) {
			SprayComps scomps =
			    SprayParticleContainer::getSprayComps();
			MultiFab::Copy(mf_plt[lev], *m_spraysource[lev].get(),
				       scomps.rhoSrcIndx, cnt++, 1, 0);
			MultiFab::Copy(mf_plt[lev], *m_spraysource[lev].get(),
				       scomps.engSrcIndx, cnt++, 1, 0);
			MultiFab::Copy(mf_plt[lev], *m_spraysource[lev].get(),
				       scomps.momSrcIndx, cnt, AMREX_SPACEDIM,
				       0);
			cnt += AMREX_SPACEDIM;
			for (int spf = 0; spf < SPRAY_FUEL_NUM; ++spf) {
				MultiFab::Copy(
				    mf_plt[lev], *m_spraysource[lev].get(),
				    scomps.specSrcIndx + spf, cnt++, 1, 0);
			}
		}
#endif
#ifdef PELE_USE_EFIELD
		if (m_do_extraEFdiags) {
			MultiFab::Copy(mf_plt[lev], *m_ionsFluxes[lev], 0, cnt,
		...
		if (m_do_les && m_plot_les) {
			...
			AMREX_D_TERM(
			    auto const& mut_arr_x = m_leveldata_old[lev]
							->visc_turb_fc[0]
							.const_arrays();
			    , auto const& mut_arr_y = m_leveldata_old[lev]
							  ->visc_turb_fc[1]
							  .const_arrays();
			    , auto const& mut_arr_z = m_leveldata_old[lev]
							  ->visc_turb_fc[2]
							  .const_arrays();)
			...
			amrex::ParallelFor(
			    mf_plt[lev],
			    [plot_arr,
			     AMREX_D_DECL(mut_arr_x, mut_arr_y, mut_arr_z),
			     cnt] AMREX_GPU_DEVICE(int box_no, int i, int j,
						   int k) noexcept {
				    plot_arr[box_no](i, j, k, cnt) =
					fact *
					(AMREX_D_TERM(
					    mut_arr_x[box_no](i, j, k) +
						mut_arr_x[box_no](i + 1, j, k),
					    +mut_arr_y[box_no](i, j, k) +
						mut_arr_y[box_no](i, j + 1, k),
					    +mut_arr_z[box_no](i, j, k) +
						mut_arr_z[box_no](i, j,
								  k + 1)));
			    });
	...
	if (do_spray_particles) {
		...
		for (int lev = 0; lev <= finest_level; ++lev) {
			SprayPC->SprayParticleIO(lev, is_spraycheck,
						 plotfilename);
			// Remove virtual particles that were made for derived
			// variables
...
void PeleLM::WriteHeader(const std::string& name, bool is_checkpoint) const {
	if (ParallelDescriptor::IOProcessor()) {
		...
		HeaderFile.rdbuf()->pubsetbuf(io_buffer.dataPtr(),
					      io_buffer.size());
...
void PeleLM::ReadCheckPointFile() {
	...
	****************************************************************************/
	...
	for (int lev = 0; lev <= max_level; ++lev) {
		SetGeometry(
		    lev, Geometry(Geom(lev).Domain(), rb, Geom(lev).CoordInt(),
				  Geom(lev).isPeriodic()));
	}
	...
	for (int lev = finest_level + 1; lev <= chk_finest_level; ++lev) {
		// read dummy level 'lev' BoxArray if restarting with reduced
		// levels
		BoxArray ba;
		ba.readFrom(is);
	}
	...
	for (int lev = 0; lev <= finest_level; ++lev) {
		...
		if (!m_restart_nonEF) {
			...
			// The chk state is 2 component shorter since phiV and
			// nE aren't in it
			MultiFab stateTemp(grids[lev], dmap[lev], NVAR - 2,
					   m_nGrowState);
			VisMF::Read(stateTemp, amrex::MultiFabFileFullPrefix(
						   lev, m_restart_chkfile,
						   level_prefix, "state"));
			MultiFab::Copy(m_leveldata_new[lev]->state, stateTemp,
				       0, 0, NVAR - 2, m_nGrowState);
		}
#else
		...
		VisMF::Read(m_leveldata_new[lev]->press,
			    amrex::MultiFabFileFullPrefix(
				lev, m_restart_chkfile, level_prefix, "p"));
		...
		if (!m_incompressible) {
			...
			if (!m_restart_nonEF) {
				...
				// I_R for non-EF simulation is one component
				// shorted, need to account for that.
				if (m_do_react) {
					MultiFab I_Rtemp(grids[lev], dmap[lev],
							 NUM_SPECIES, 0);
					VisMF::Read(
					    I_Rtemp,
					    amrex::MultiFabFileFullPrefix(
						...
						level_prefix, "I_R"));
					MultiFab::Copy(
					    m_leveldatareact[lev]->I_R, I_Rtemp,
					    0, 0, NUM_SPECIES, 0);
				}

				// Initialize nE & phiV
				m_leveldata_new[lev]->state.setVal(
				    0.0, NE, 2, m_nGrowState);
			}
#else
			if (m_do_react) {
...
void PeleLM::initLevelDataFromPlt(int a_lev, const std::string& a_dataPltFile) {
	if (m_incompressible) {
		Abort(
		    " initializing data from a pltfile only available for "
	...
	if (pltfileSource == "LM") {
		amrex::Print()
		    << " Assuming pltfile was generated in LM/LMeX \n";
	...
	for (int i = 0; i < plt_vars.size(); ++i) {
		...
		if (firstChars == "Y(" &&
		    idY < 0) {	// species might not be ordered in the order of
				// the current mech.
			idY = i;
		}
	...
	for (int i = 0; i < NUM_SPECIES; i++) {
		...
		for (int iplt = 0; iplt < nSpecPlt; iplt++) {
			if (specString == plt_vars[idY + iplt]) {
				MultiFab::Copy(ldata_p->state, speciesPlt, iplt,
					       FIRSTSPEC + i, 1, 0);
				foundSpec = 1;
			}
		}
...
#endif
		for (MFIter mfi(ldata_p->state, TilingIfNotGPU());
		     mfi.isValid(); ++mfi) {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
				for (MFIter mfi(ldata_p->state,
						TilingIfNotGPU());
				     mfi.isValid(); ++mfi) {
					const Box& bx = mfi.tilebox();
					auto const& soot_arr =
					    ldata_p->state.array(mfi,
								 FIRSTSOOT);
					amrex::ParallelFor(
					    bx,
					    [=] AMREX_GPU_DEVICE(
			...
			for (int mom = 0; mom < NUM_SOOT_MOMENTS + 1; ++mom) {
				ldata_p->state.setVal(moments[mom],
						      FIRSTSOOT + mom, 1);
			}
		}
	}
	...
	// The above handles species mapping (to some extent), but nothing
	// enforce sum of Ys = 1 -> use N2 in the following if N2 is present
#ifdef AMREX_USE_OMP
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
