...
void
DiagPDF::init(const std::string &a_prefix,
...
void
DiagPDF::processDiag(int a_nstep,
                     ...
                     const amrex::Vector<std::string> &a_stateVar)
{
    ...
    for (int lev = 0; lev < a_state.size(); ++lev) {
        ...
        if (m_volWeighted) {
            ...
            amrex::ParallelFor(*a_state[lev], amrex::IntVect(0),
                [=,lowBnd=m_lowBnd] AMREX_GPU_DEVICE(int box_no, int i, int j, int k) noexcept {
                    if (marrs[box_no](i,j,k)) {
                        int cbin = std::floor((sarrs[box_no](i,j,k,fieldIdx) - lowBnd) / binWidth);
                        amrex::HostDevice::Atomic::Add(&(pdf_d_p[cbin]), varrs[box_no](i,j,k));
                    }
                });
        } else {
            auto *pdf_d_p = pdf_d.dataPtr();
...
amrex::Real
DiagPDF::MFVecMin(const amrex::Vector<const amrex::MultiFab*> &a_state,
                  int comp)
{
...
amrex::Real
DiagPDF::MFVecMax(const amrex::Vector<const amrex::MultiFab*> &a_state,
                  int comp)
{
    // TODO: skip fine-covered in search
