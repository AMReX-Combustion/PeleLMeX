  ...
  const amrex::DistributionMapping& dm)
{
  ...
  if (!m_incompressible) {
      ...
      const amrex::DistributionMapping& dm)
    {
      ...
      if (!m_incompressible) {
        ...
        if (m_do_extraEFdiags) {
          m_ionsFluxes[lev].reset(
            new MultiFab(ba, dm, NUM_IONS * AMREX_SPACEDIM, 0));
        }
        m_precond_op.reset();
#endif
      ...
      void PeleLM::computeCosts(
        int a_lev, LayoutData<Real>& a_costs, int a_costMethod)
      {
      ...
      for (int lev = lbase + 1; lev <= new_finest; ++lev) {
        if (lev <= finest_level) // an old level
        {
          ...
          // If initial grid generation or not doing load balancing,
          // remake level only if the grid changed using the default DMap
          // strategy
          if (initial || !m_doLoadBalance) {
            if (ba_changed || coarse_ba_changed) {
              BoxArray level_grids = grids[lev];
            ...
            // When doing load balancing, remake level if grid changed or a
            // better dmap can be obtained
          } else {
  ...
  void PeleLM::MakeNewLevelFromCoarse(
    int lev, amrex::Real time, const amrex::BoxArray& ba,
    const amrex::DistributionMapping& dm)
  {
  ...
  void PeleLM::regridFromGridFile(int lbase, amrex::Real time, bool /*initial*/)
  {
