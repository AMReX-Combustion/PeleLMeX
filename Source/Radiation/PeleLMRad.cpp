...
#endif
		for (amrex::MFIter mfi(*(m_extSource[lev]),
				       amrex::TilingIfNotGPU());
		     mfi.isValid(); ++mfi) {
			auto const& q_yin_co2 = ldata_p->state.const_array(
			    mfi, FIRSTSPEC + co2Indx);
			auto const& q_yin_h2o = ldata_p->state.const_array(
			    mfi, FIRSTSPEC + h2oIndx);
			auto const& q_yin_co =
			    ldata_p->state.const_array(mfi, FIRSTSPEC + coIndx);
			auto const& q_Tin =
			    ldata_p->state.const_array(mfi, TEMP);
			auto const& q_Pin =
			    ldata_p->state.const_array(mfi, RHORT);
#ifdef PELELM_USE_SOOT
			auto const& q_fvin =
			    ldata_p->state.const_array(mfi, FIRSTSOOT + 1);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
		...
		for (amrex::MFIter mfi(*(m_extSource[lev]),
				       amrex::TilingIfNotGPU());
		     mfi.isValid(); ++mfi) {
			auto const& source_arr =
			    m_extSource[lev]->array(mfi, RHOH);
