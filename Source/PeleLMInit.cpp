...
void PeleLM::MakeNewLevelFromScratch(int lev, amrex::Real time,
				     const amrex::BoxArray& ba,
				     const amrex::DistributionMapping& dm) {
	BL_PROFILE_VAR("PeleLM::MakeNewLevelFromScratch()",
	...
	if (m_verbose > 0) {
		...
		if (m_verbose > 2 && lev > 0) {
			...
			amrex::Print()
			    << " with " << ba.numPts() << " cells," << ba.size()
			    << " boxes,"
			    << " over "
			    << ba.numPts() * vol / geom[0].ProbSize() * 100
			    << "% of the domain \n";
		}
	...
	m_leveldata_old[lev].reset(new LevelData(
	    grids[lev], dmap[lev], *m_factory[lev], m_incompressible,
	    m_has_divu, m_nAux, m_nGrowState, m_use_soret, m_do_les));
	...
	if (max_level > 0 && lev != max_level) {
		m_coveredMask[lev].reset(
		    new iMultiFab(grids[lev], dmap[lev], 1, 0));
	...
	if (m_do_extraEFdiags) {
		m_ionsFluxes[lev].reset(new MultiFab(
		    grids[lev], dmap[lev], NUM_IONS * AMREX_SPACEDIM, 0));
	}
#endif
	...
	m_extSource[lev].reset(new MultiFab(grids[lev], dmap[lev], NVAR,
					    amrex::max(m_nGrowAdv, m_nGrowMAC),
	...
	if (lev == 0 && m_signDistNeeded) {
		...
		m_signedDist0.reset(new MultiFab(grids[lev], dmap[lev], 1, 1,
						 MFInfo(), *m_factory[lev]));
		...
		for (int ilev = 1; ilev <= max_level; ++ilev) {
			extentFactor +=
			    ...
			    std::pow(static_cast<Real>(refRatio(ilev - 1)[0]),
				     static_cast<Real>(ilev));
		}
		extentFactor *= std::sqrt(2.0) *
				m_derefineEBBuffer;  // Account for diagonals

		MultiFab signDist(convert(grids[0], IntVect::TheUnitVector()),
				  dmap[0], 1, 1, MFInfo(), EBFactory(0));
		FillSignedDistance(signDist, true);

#ifdef AMREX_USE_OMP
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
		for (MFIter mfi(*m_signedDist0, TilingIfNotGPU());
		     mfi.isValid(); ++mfi) {
			const Box& bx = mfi.growntilebox();
			auto const& sd_cc = m_signedDist0->array(mfi);
			auto const& sd_nd = signDist.const_array(mfi);
			amrex::ParallelFor(
			    bx, [sd_cc, sd_nd] AMREX_GPU_DEVICE(
...
void PeleLM::initData() {
	...
	if (m_restart_chkfile.empty()) {
		...
		if (!m_initial_grid_file.empty()) {
			...
			// This is an AmrCore member function which recursively
			// makes new levels with MakeNewLevelFromScratch.
		...
		if (m_plot_int > 0 || m_plot_per_approx > 0. ||
		    m_plot_per_exact > 0.) {
			WritePlotFile();
		}
		...
		if (m_restart_nonEF) {
			// either pass Y_ne -> nE or initialize nE for
			// electro-neutral
		...
		if (m_regrid_on_restart) {
			...
			for (int lev{finest_level}; lev < max_level; ++lev) {
				regrid(0, m_cur_time);
				// Need to fill the old state to enable regrid
				// on higher levels
				copyStateNewToOld(1);
				copyPressNewToOld();
				if (m_do_react) {
					auto ldataR_p =
					    getLevelDataReactPtr(lev);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
	...
	for (MFIter mfi(ldata_p->state, TilingIfNotGPU()); mfi.isValid();
		...
		auto const& aux_arr = (m_nAux > 0)
					  ? ldata_p->auxiliaries.array(mfi)
					  : DummyFab.array();
		amrex::ParallelFor(
		    bx,
		    [=, m_incompressible = m_incompressible] AMREX_GPU_DEVICE(
			int i, int j, int k) noexcept {
			    pelelm_initdata(i, j, k, m_incompressible,
					    state_arr, aux_arr, geomdata,
					    *lprobparm, lpmfdata);
		    });
	}
...
void PeleLM::projectInitSolution() {
	...
	if (m_do_init_proj) {
		...
		// reaction divU which can make the dt for evaluating I_R better
		if (m_has_divu) {
			...
			diffData.reset(new AdvanceDiffData(
			    finest_level, grids, dmap, m_factory, m_nGrowAdv,
			    m_use_wbar, m_use_soret, is_initialization));
			calcDivU(is_initialization, computeDiffusionTerm,
				 do_avgDown, AmrNewTime, diffData);
		}
		initialProjection();

		// If gravity is used, do initial pressure projection to get the
		...
		for (int iter = 0; iter < m_numDivuIter; iter++) {
			if (m_do_react) {
				...
				// Copy new -> old since old used in
				// advanceChemistry
				copyStateNewToOld();

				for (int lev = finest_level; lev >= 0; --lev) {
					// Setup empty forcing
					MultiFab Forcing(grids[lev], dmap[lev],
							 nCompForcing(), 0);
					Forcing.setVal(0.0);

					if (lev != finest_level) {
						advanceChemistryBAChem(
						    lev, dtInit / 2.0, Forcing);
					} else {
						if (m_max_grid_size_chem.min() >
						    0) {
							advanceChemistryBAChem(
							    lev, dtInit / 2.0,
							    Forcing);
			...
			if (m_has_divu) {
				...
				std::unique_ptr<AdvanceDiffData> diffData;
				diffData.reset(new AdvanceDiffData(
				    finest_level, grids, dmap, m_factory,
				    m_nGrowAdv, m_use_wbar, m_use_soret,
				    is_initialization));
				calcDivU(is_initialization,
					 computeDiffusionTerm, do_avgDown,
					 AmrNewTime, diffData);
			}
			initialProjection();
		}
...
void PeleLM::checkRunParams() {
	...
	if (geom[0].IsRZ() && m_phys_bc.lo(0) != 3) {
		Abort(
		    "x-low must be 'Symmetry' when using RZ coordinate system");
	}
#endif
}
