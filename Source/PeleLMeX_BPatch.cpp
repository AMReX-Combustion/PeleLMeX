...
#else
	if(m_patchtype!="full-boundary" && m_patchtype!="circle" && m_patchtype!="rectangle" && m_patchtype!="circle-annular" && m_patchtype!="rectangle-annular"){
		amrex::Abort("\nPatch type do not match allowed values (full-boundary,circle,rectangle,circle-annular,rectangle-annular) \n");
	}
#endif

	if(m_bpdata_h.m_boundary_dir!=0 && m_bpdata_h.m_boundary_dir!=1 && m_bpdata_h.m_boundary_dir!=2)
	{
