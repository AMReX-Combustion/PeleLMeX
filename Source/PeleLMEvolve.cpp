...
void PeleLM::Evolve() {
	...
	while (!do_not_evolve) {
		...
		if ((m_regrid_int > 0) && (m_nstep > 0) &&
		    (m_nstep % m_regrid_int == 0)) {
			if (m_verbose > 0) amrex::Print() << " Regridding...\n";
			// Average down I_R to have proper values in newly
			// uncovered areas
		...
		if (m_max_wall_time > 0.0) {
			amrex::Real t_elapsed =
			    ParallelDescriptor::second() - m_wall_start;
		...
		do_not_evolve =
		    ((m_max_step >= 0 && m_nstep >= m_max_step) ||
		     (m_stop_time >= 0.0 &&
		      m_cur_time >= m_stop_time - 1.0e-12 * m_dt) ||
...
bool PeleLM::writePlotNow() {
	bool write_now = false;

	if (m_plot_int > 0 && (m_nstep % m_plot_int == 0)) {
		write_now = true;

	} else if (m_plot_per_exact > 0.0 &&
		   (std::abs(std::remainder(m_cur_time, m_plot_per_exact)) <
		    1.e-12)) {
		write_now = true;

	} else if (m_plot_per_approx > 0.0) {
		// Check to see if we've crossed a plot_per interval by
		// comparing the number of intervals that have elapsed for both
		// the current time and the time at the beginning of this
		// timestep.
		int num_per_old =
		    static_cast<int>((m_cur_time - m_dt) / m_plot_per_approx);
		int num_per_new =
		    static_cast<int>((m_cur_time) / m_plot_per_approx);
		// Before using these, however, we must test for the case where
		// we're within machine epsilon of the next interval. In that
		// case, increment the counter, because we have indeed reached
		// the next plot_per interval at this point.
		const Real eps = std::numeric_limits<Real>::epsilon() *
				 10.0_rt * std::abs(m_cur_time);
		const Real next_plot_time =
		    (num_per_old + 1) * m_plot_per_approx;
		if ((num_per_new == num_per_old) &&
		    std::abs(m_cur_time - next_plot_time) <= eps) {
			num_per_new += 1;
		}
		// Similarly, we have to account for the case where the old time
		// is within machine epsilon of the beginning of this interval,
		// so that we don't double count that time threshold -- we
		// already plotted at that time on the last timestep.
		if ((num_per_new != num_per_old) &&
		    std::abs((m_cur_time - m_dt) - next_plot_time) <= eps) {
			num_per_old += 1;
		}
...
bool PeleLM::writeCheckNow() {
	...
	if (m_check_int > 0 && (m_nstep % m_check_int == 0)) {
		...
		// timestep.
		int num_per_old =
		    static_cast<int>((m_cur_time - m_dt) / m_check_per);
		...
		// already plotted at that time on the last timestep.
		if ((num_per_new != num_per_old) &&
		    std::abs((m_cur_time - m_dt) - next_check_time) <= eps) {
			num_per_old += 1;
		}
		if (num_per_old != num_per_new) {
			write_now = true;
...
bool PeleLM::checkMessage(const std::string &a_action) {
