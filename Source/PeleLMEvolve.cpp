      ...
      if (t_elapsed >= (m_max_wall_time * 3600)) {
        ...
        if (m_verbose > 0) {
          amrex::Print() << std::endl
                         << "Reached maxmimum allowed wall time, stopping ..."
                         << std::endl;
        }
      }
    }
...
PeleLM::writePlotNow()
{
  ...
  if (m_plot_int > 0 && (m_nstep % m_plot_int == 0)) {
    ...
    // machine epsilon of the beginning of this interval, so that we don't
    // double count that time threshold -- we already plotted at that time on
    // the last timestep.
    if (
      (num_per_new != num_per_old) &&
