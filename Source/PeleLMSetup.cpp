...
PeleLM::readParameters() {
  ...
  for (int idim = 0; idim < AMREX_SPACEDIM; idim++) {
    if (lo_bc_char[idim] == "Neutral") {
      ...
      amrex::Abort("Wrong PhiV polarity. Should be : Neutral, Anode "
                   "or Cathode");
    }
  ...
  if (mgsc_size > 0) {
    if (mgsc_size == 1) {
      ...
      Abort("peleLM.max_grid_size_chem should have 1 or "
  ...
  if (m_advection_key == "Godunov_PLM") {
    ...
    Abort("Unknown 'advection_scheme'. Recognized options are: "
  ...
  if (max_level > 0) {
    ...
    if (m_EB_refine_type != "Static" && m_EB_refine_type != "Adaptive") {
      Abort("refine_EB_type can only be 'Static' or "
            "'Adaptive'");
    }
...
#define STRIP                \
  ...
  if (in_finest > max_level) {
    amrex::Error("You have fewer levels in your inputs file then in your "
                 "grids file!");
  }
...
PeleLM::derivedSetup() {
  ...
  if (!m_incompressible) {
    ...
    // Rho - sum rhoYs
    derive_lst.add(
      "rhominsumrhoY", IndexType::TheCellType(), 1, pelelm_derrhomrhoy,
      the_same_box);

    // Heat Release
    derive_lst.add(
      "HeatRelease", IndexType::TheCellType(), 1, pelelm_derheatrelease,
      the_same_box);

    // Thermal diffusivity
    derive_lst.add(
      "lambda", IndexType::TheCellType(), 1, pelelm_derlambda, the_same_box);

    // Mixture fraction
    derive_lst.add(
      "mixture_fraction", IndexType::TheCellType(), 1, pelelm_dermixfrac,
      the_same_box);

    // Progress variable
    derive_lst.add(
      "progress_variable", IndexType::TheCellType(), 1, pelelm_derprogvar,
      the_same_box);
  }

  // Distribution Map
  derive_lst.add(
    "DistributionMap", IndexType::TheCellType(), 1, pelelm_derdmap,
    the_same_box);

  // Cell average pressure
  derive_lst.add(
    "avg_pressure", IndexType::TheCellType(), 1, pelelm_deravgpress,
    the_same_box);

  // Viscosity
  derive_lst.add(
    "viscosity", IndexType::TheCellType(), 1, pelelm_dervisc, the_same_box);

  // Velocity magnitude
  derive_lst.add(
    "mag_vel", IndexType::TheCellType(), 1, pelelm_dermgvel, the_same_box);

  // Vorticity magnitude
  derive_lst.add(
    "mag_vort", IndexType::TheCellType(), 1, pelelm_dermgvort, grow_box_by_two);
  ...
  // Q-criterion
  derive_lst.add(
    "Qcrit", IndexType::TheCellType(), 1, pelelm_derQcrit, grow_box_by_two);
#endif

  // Kinetic energy
  derive_lst.add(
    "kinetic_energy", IndexType::TheCellType(), 1, pelelm_derkineticenergy,
    the_same_box);

  // Enstrophy
  derive_lst.add(
    "enstrophy", IndexType::TheCellType(), 1, pelelm_derenstrophy,
    grow_box_by_two);

#ifdef PELE_USE_EFIELD
  // Charge distribution
  derive_lst.add(
    "chargedistrib", IndexType::TheCellType(), 1, pelelm_derchargedist,
    the_same_box);

  // Electric field
  derive_lst.add(
    "efieldx", IndexType::TheCellType(), 1, pelelm_derefx, grow_box_by_one);
#if (AMREX_SPACEDIM > 1)
  derive_lst.add(
    "efieldy", IndexType::TheCellType(), 1, pelelm_derefy, grow_box_by_one);
#if (AMREX_SPACEDIM > 2)
  derive_lst.add(
    "efieldz", IndexType::TheCellType(), 1, pelelm_derefz, grow_box_by_one);
#endif
#endif
  // Lorentz forces
  derive_lst.add(
    "LorentzFx", IndexType::TheCellType(), 1, pelelm_derLorentzx,
    grow_box_by_one);
#if (AMREX_SPACEDIM > 1)
  derive_lst.add(
    "LorentzFy", IndexType::TheCellType(), 1, pelelm_derLorentzy,
    grow_box_by_one);
#if (AMREX_SPACEDIM > 2)
  derive_lst.add(
    "LorentzFz", IndexType::TheCellType(), 1, pelelm_derLorentzz,
    grow_box_by_one);
#endif
#endif
#endif
