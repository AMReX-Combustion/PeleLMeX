...
void PeleLM::Setup() {
	...
	// Ensure grid is isotropic
	{
		...
		AMREX_ALWAYS_ASSERT(
		    AMREX_D_TERM(, amrex::almostEqual(dx[0], dx[1]),
	...
	if (!m_incompressible) {
		...
		if (m_do_react) {
			...
			m_reactor =
			    pele::physics::reactions::ReactorBase::create(
				m_chem_integrator);
			m_reactor->init(reactor_type, ncells_chem);

			// For ReactorNull, we need to also skip instantaneous
			// RR used in divU
...
void PeleLM::readParameters() {
	...
	if (verbose && m_closed_chamber) {
		Print() << " Simulation performed with the closed chamber "
			   "algorithm \n";
	}
	...
	for (int idim = 0; idim < AMREX_SPACEDIM; idim++) {
		if (lo_bc_char[idim] == "Interior") {
			...
			amrex::Abort(
			    "Wrong PhiV bc. Should be : Interior, Dirichlet or "
			    "Neumann");
		}
	...
	for (int idim = 0; idim < AMREX_SPACEDIM; idim++) {
		if (lo_bc_char[idim] == "Neutral") {
			...
			amrex::Abort(
			    "Wrong PhiV polarity. Should be : Neutral, Anode "
			    "or Cathode");
		}
	...
	if (m_incompressible) {
		...
		AMREX_ASSERT_WITH_MESSAGE(
		    m_rho > 0.0,
		    "peleLM.rho is needed when running incompressible");
		AMREX_ASSERT_WITH_MESSAGE(
		    m_mu > 0.0,
		    "peleLM.mu is needed when running incompressible");
	}
	...
	if (m_les_model == "None") {
		...
		if (m_les_model == "Smagorinsky") {
			...
			AMREX_ALWAYS_ASSERT(AMREX_SPACEDIM ==
					    3);	 // Sigma only available in 3D
		} else {
			amrex::Abort(
			    "LES model must be None, Smagorinsky, WALE or "
			    "Sigma. Invalid choie: " +
			    m_les_model);
		}
		m_do_les = true;
...
#ifdef PELE_USE_EFIELD
		amrex::Abort(
		    "LES implementation is not yet compatible with "
		    "efield/ions");
#endif
	}
	...
	if (m_use_wbar and m_unity_Le) {
		m_use_wbar = 0;
		amrex::Print()
		    << "WARNING: use_wbar set to false because unity_Le is true"
		    << std::endl;
	}
	...
	if (mgsc_size > 0) {
		if (mgsc_size == 1) {
			...
			Vector<int> mgsc;
			pp.getarr("max_grid_size_chem", mgsc, 0,
				  AMREX_SPACEDIM);
			...
			Abort(
			    "peleLM.max_grid_size_chem should have 1 or "
			    "AMREX_SPACEDIM values");
		}
	}
	...
	parseUserKey(pp, "load_balancing_method", lbmethod,
		     m_loadBalanceMethod);
	parseUserKey(pp, "load_balancing_cost_estimate", lbcost,
		     m_loadBalanceCost);
	pp.query("load_balancing_efficiency_threshold",
		 m_loadBalanceEffRatioThreshold);
	parseUserKey(pp, "chem_load_balancing_method", lbmethod,
		     m_loadBalanceMethodChem);
	parseUserKey(pp, "chem_load_balancing_cost_estimate", lbcost,
		     m_loadBalanceCostChem);
	...
	if (m_advection_key == "Godunov_PLM") {
		...
		Abort(
		    "Unknown 'advection_scheme'. Recognized options are: "
		    "Godunov_PLM, Godunov_PPM or Godunov_BDS");
	}
	...
	ppnproj.query("mg_max_coarsening_level",
	...
	ppmacproj.query("mg_max_coarsening_level",
			m_mac_mg_max_coarsening_level);
	...
	if (max_level > 0) {
		...
		if (m_EB_refine_type != "Static" &&
			...
			Abort(
			    "refine_EB_type can only be 'Static' or "
			    "'Adaptive'");
		}
	...
	if (runMode() == "evaluate") {
		...
		if (m_evaluatePlotVarCount != 0) {
			...
			for (int ivar = 0; ivar < m_evaluatePlotVarCount;
			     ivar++) {
				pp.get("evaluate_vars",
				       m_evaluatePlotVars[ivar], ivar);
			}
		}
	}
...
void PeleLM::readIOParameters() {
	...
	if (m_derivePlotVarCount != 0) {
		...
		for (int ivar = 0; ivar < m_derivePlotVarCount; ivar++) {
			pp.get("derive_plot_vars", m_derivePlotVars[ivar],
			       ivar);
		}
	}
	...
	AMREX_ASSERT(m_regrid_interp_method == 0 ||
...
void PeleLM::variablesSetup() {
	...
	//----------------------------------------------------------------
	// Variables ordering is defined through compiler macro in
	// PeleLM_Index.H Simply print on screen the state layout and append to
	// the stateComponents list
	Print() << "\n";
	Print() << PrettyLine;
	...
	if (!m_incompressible) {
		...
		pele::physics::eos::speciesNames<
		    pele::physics::PhysicsType::eos_type>(names);
		for (int n = 0; n < NUM_SPECIES; n++) {
			stateComponents.emplace_back(FIRSTSPEC + n,
						     "rho.Y(" + names[n] + ")");
			reactComponents.emplace_back(n,
						     "I_R(" + names[n] + ")");
		}
		Print() << " Enthalpy: " << RHOH << "\n";
		...
		for (int mom = 0; mom < NUMSOOTVAR; mom++) {
			std::string sootname =
			    soot_model->sootVariableName(mom);
	...
	if (m_nAux > 0) {
		...
		for (int n = 0; n < m_nAux; n++) {
			stateComponents.emplace_back(
			    FIRSTAUX + n, "Aux_" + std::to_string(n));
		}
	}

	if (m_incompressible) {
		Print() << " => Total number of state variables: "
			<< AMREX_SPACEDIM << "\n";
	} else {
	...
	if (max_level > 0 && !m_regrid_file.empty() && m_regrid_int > 0) {
		...
		if (verbose > 0) {
			amrex::Print() << "Read regrid_ba. Size is "
				       << m_regrid_ba.size() << "\n";
		}
	}
}
...
#define STRIP                      \
	...
	if (in_finest > max_level) {
		amrex::Error(
		    "You have fewer levels in your inputs file then in your "
		    "grids file!");
	}
...
void PeleLM::derivedSetup() {
	...
	if (!m_incompressible) {
		...
		derive_lst.add("mass_fractions", IndexType::TheCellType(),
			       NUM_SPECIES, var_names_massfrac,
			       pelelm_dermassfrac, the_same_box);

		for (int n = 0; n < NUM_SPECIES; n++) {
			var_names_massfrac[n] = "X(" + spec_names[n] + ")";
		}
		derive_lst.add("mole_fractions", IndexType::TheCellType(),
			       NUM_SPECIES, var_names_massfrac,
			       pelelm_dermolefrac, the_same_box);
		...
		if (m_use_soret) {
			...
			derive_lst.add("diffcoeff", IndexType::TheCellType(),
				       2 * NUM_SPECIES, var_names_massfrac,
				       pelelm_derdiffc, the_same_box);
		} else {
			derive_lst.add("diffcoeff", IndexType::TheCellType(),
				       NUM_SPECIES, var_names_massfrac,
				       pelelm_derdiffc, the_same_box);
		...
		// Rho - sum rhoYs
		derive_lst.add("rhominsumrhoY", IndexType::TheCellType(), 1,
			       pelelm_derrhomrhoy, the_same_box);

		// Heat Release
		derive_lst.add("HeatRelease", IndexType::TheCellType(), 1,
			       pelelm_derheatrelease, the_same_box);

		// Thermal diffusivity
		derive_lst.add("lambda", IndexType::TheCellType(), 1,
			       pelelm_derlambda, the_same_box);

		// Mixture fraction
		derive_lst.add("mixture_fraction", IndexType::TheCellType(), 1,
			       pelelm_dermixfrac, the_same_box);

		// Progress variable
		derive_lst.add("progress_variable", IndexType::TheCellType(), 1,
			       pelelm_derprogvar, the_same_box);
	}

	// Distribution Map
	derive_lst.add("DistributionMap", IndexType::TheCellType(), 1,
		       pelelm_derdmap, the_same_box);

	// Cell average pressure
	derive_lst.add("avg_pressure", IndexType::TheCellType(), 1,
		       pelelm_deravgpress, the_same_box);

	// Viscosity
	derive_lst.add("viscosity", IndexType::TheCellType(), 1, pelelm_dervisc,
		       the_same_box);

	// Velocity magnitude
	derive_lst.add("mag_vel", IndexType::TheCellType(), 1, pelelm_dermgvel,
		       the_same_box);

	// Vorticity magnitude
	derive_lst.add("mag_vort", IndexType::TheCellType(), 1,
		       pelelm_dermgvort, grow_box_by_two);

	// Spatial coordinates
	{
		Vector<std::string> var_names({AMREX_D_DECL("x", "y", "z")});
		derive_lst.add("coordinates", IndexType::TheCellType(),
			       AMREX_SPACEDIM, var_names, pelelm_dercoord,
			       the_same_box);
	}
...
#endif
		derive_lst.add("vorticity", IndexType::TheCellType(),
			       vort_ncomp, var_names, pelelm_dervort,
			       grow_box_by_two);
	}
	...
	// UserDefined derived
	{
		...
		derive_lst.add("derUserDefined", IndexType::TheCellType(),
			       var_names.size(), var_names, pelelm_deruserdef,
			       the_same_box);
	}

#if (AMREX_SPACEDIM == 3)
	// Q-criterion
	derive_lst.add("Qcrit", IndexType::TheCellType(), 1, pelelm_derQcrit,
		       grow_box_by_two);
#endif

	// Kinetic energy
	derive_lst.add("kinetic_energy", IndexType::TheCellType(), 1,
		       pelelm_derkineticenergy, the_same_box);

	// Enstrophy
	derive_lst.add("enstrophy", IndexType::TheCellType(), 1,
		       pelelm_derenstrophy, grow_box_by_two);

#ifdef PELE_USE_EFIELD
	// Charge distribution
	derive_lst.add("chargedistrib", IndexType::TheCellType(), 1,
		       pelelm_derchargedist, the_same_box);

	// Electric field
	derive_lst.add("efieldx", IndexType::TheCellType(), 1, pelelm_derefx,
		       grow_box_by_one);
#if (AMREX_SPACEDIM > 1)
	derive_lst.add("efieldy", IndexType::TheCellType(), 1, pelelm_derefy,
		       grow_box_by_one);
#if (AMREX_SPACEDIM > 2)
	derive_lst.add("efieldz", IndexType::TheCellType(), 1, pelelm_derefz,
		       grow_box_by_one);
#endif
#endif
	// Lorentz forces
	derive_lst.add("LorentzFx", IndexType::TheCellType(), 1,
		       pelelm_derLorentzx, grow_box_by_one);
#if (AMREX_SPACEDIM > 1)
	derive_lst.add("LorentzFy", IndexType::TheCellType(), 1,
		       pelelm_derLorentzy, grow_box_by_one);
#if (AMREX_SPACEDIM > 2)
	derive_lst.add("LorentzFz", IndexType::TheCellType(), 1,
		       pelelm_derLorentzz, grow_box_by_one);
#endif
#endif
#endif
#ifdef PELELM_USE_SOOT
...
void PeleLM::evaluateSetup() {
	...
	// projected velocity field
	{
		...
		evaluate_lst.add("velProj", IndexType::TheCellType(),
				 AMREX_SPACEDIM, var_names, the_same_box);
	}
	...
	// velocity force
	{
		...
		evaluate_lst.add("velForce", IndexType::TheCellType(),
				 AMREX_SPACEDIM, var_names, the_same_box);
	}
	...
	// divTau
	{
		...
		evaluate_lst.add("divTau", IndexType::TheCellType(),
				 AMREX_SPACEDIM, var_names, the_same_box);
	}
	...
	// scalar diffusion term
	{
		...
		evaluate_lst.add("diffTerm", IndexType::TheCellType(),
				 NUM_SPECIES + 2, var_names, the_same_box);
	}
	...
	// advection terms
	{
		...
		evaluate_lst.add("advTerm", IndexType::TheCellType(), NVAR - 2,
	...
	// Chemical state and external chem. forcing (used in ReactEval)
	{
		...
		for (int n = 0; n < NUM_SPECIES; n++) {
			var_names[NUM_SPECIES + 2 + n] =
			    "F_rhoY(" + spec_names[n] + ")";
		}
		var_names[2 * NUM_SPECIES + 2] = "F_rhoH";
		evaluate_lst.add("chemTest", IndexType::TheCellType(),
				 2 * (NUM_SPECIES + 1) + 1, var_names,
				 the_same_box);
	}
	...
	// instantaneous reaction rate
	{
		...
		evaluate_lst.add("instRR", IndexType::TheCellType(),
				 NUM_SPECIES, var_names, the_same_box);
	}
	...
	// cell-centered transport coefficients
	{
		...
		evaluate_lst.add("transportCC", IndexType::TheCellType(),
				 NUM_SPECIES + 2, var_names, the_same_box);
	}
}
...
void PeleLM::taggingSetup() {
	...
	for (int n = 0; n < refinement_indicators.size(); ++n) {
		std::string ref_prefix =
		    amr_prefix + "." + refinement_indicators[n];
		...
		if (ppr.countval("value_greater")) {
			...
			errTags.push_back(AMRErrorTag(
			    value, AMRErrorTag::GREATER, field, info));
			itexists = derive_lst.canDerive(field) ||
				   isStateVariable(field) ||
			...
			ppr.get("field_name", field);
			errTags.push_back(
			    AMRErrorTag(value, AMRErrorTag::LESS, field, info));
			itexists = derive_lst.canDerive(field) ||
				   isStateVariable(field) ||
				   isReactVariable(field);
			...
			errTags.push_back(
			    AMRErrorTag(value, AMRErrorTag::VORT, field, info));
			itexists = derive_lst.canDerive(field) ||
				   isStateVariable(field) ||
				   isReactVariable(field);
			...
			ppr.get("field_name", field);
			errTags.push_back(
			    AMRErrorTag(value, AMRErrorTag::GRAD, field, info));
			itexists = derive_lst.canDerive(field) ||
				   isStateVariable(field) ||
				   isReactVariable(field);
		} else if (realbox.ok()) {
		...
		if (!itexists) {
			amrex::Error(
			    "PeleLM::taggingSetup(): unknown variable field "
			    "for criteria " +
			    refinement_indicators[n]);
		}
	}
...
void PeleLM::resizeArray() {
	if (m_verbose) {
		Print() << " Initializing data for " << max_level + 1
			<< " levels \n";
	}
