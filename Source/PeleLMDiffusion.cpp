...
#pragma omp parallel if (Gpu::notInLaunchRegion())
          ...
          for (MFIter mfi(Enth, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            ...
            } else if (flagfab.getType(gbx) != FabType::regular) { // EB
                                                                   // containing
                                                                   // boxes
              amrex::ParallelFor(
                gbx, [Temp_arr, Hi_arr,
...
#endif
            {
              amrex::ParallelFor(
                gbx, [Temp_arr,
                      Hi_arr] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                  getHGivenT(i, j, k, Temp_arr, Hi_arr);
                });
            }
          }
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
            ...
            for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
              ...
              amrex::ParallelFor(
                ebox, [spflux_ar, enthflux_ar,
                  ...
                  if (need_wbar_fluxes) {
                    for (int n = 0; n < NUM_SPECIES; n++) {
                      spwbarFlux_ar(i, j, k, n) = -y[n] / WBAR *
                                                  beta_ar(i, j, k, n) *
                                                  gradWbar_ar(i, j, k);
                    }
                  }
      ...
      Vector<MultiFab const*> const& a_beta)
    {
      ...
      //------------------------------------------------------------------------
      // Compute T gradients and do average down to get gradients consistent
      // accross levels Get the temperature BCRec
      int do_avgDown = 1;
      auto bcRecTemp = fetchBCRecArray(TEMP, 1);
      auto bcRecSpec = fetchBCRecArray(FIRSTSPEC, NUM_SPECIES);
      ...
      for (int lev = 0; lev <= finest_level; ++lev) {
        ...
        int doZeroVisc = 1;
        Array<MultiFab, AMREX_SPACEDIM> beta_ec = getDiffusivity(
          lev, NUM_SPECIES + 2, NUM_SPECIES, doZeroVisc, bcRecSpec,
          *a_beta[lev]);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
        {
          FArrayBox rhoY_ed;
          FArrayBox T_ed;
          for (MFIter mfi(*a_beta[lev], TilingIfNotGPU()); mfi.isValid();
               ++mfi) {
            for (int idim = 0; idim < AMREX_SPACEDIM; idim++) {
    ...
    void PeleLM::adjustSpeciesFluxes(
      const Vector<Array<MultiFab*, AMREX_SPACEDIM>>& a_spfluxes,
      Vector<MultiFab const*> const& a_spec)
    {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
        ...
        for (MFIter mfi(*a_spec[lev], TilingIfNotGPU()); mfi.isValid(); ++mfi) {
          for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
            ...
            if (flagfab.getType(amrex::grow(ebx, 0)) != FabType::covered) {
              // No cut cells in tile + nghost-cell witdh halo -> use non-eb
              // routine
              if (
                flagfab.getType(amrex::grow(ebx, nGrow)) == FabType::regular) {
                amrex::ParallelFor(
                  ebx, [idim, rhoY, flux_dir, edomain, bc_lo,
                        bc_hi] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
                    int idx[3] = {i, j, k};
    ...
    void PeleLM::computeSpeciesEnthalpyFlux(
      const Vector<Array<MultiFab*, AMREX_SPACEDIM>>& a_fluxes,
      Vector<MultiFab const*> const& a_temp)
    {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    void PeleLM::differentialDiffusionUpdate(
      ...
      std::unique_ptr<AdvanceDiffData> & diffData)
    {
      ...
      //------------------------------------------------------------------------
      // Convert species forcing into actual solve RHS by *dt and adding
      // rhoY^{n} Could have done it at the same time the forcing is built, but
      // this is clearer
      for (int lev = 0; lev <= finest_level; ++lev) {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
          ...
          for (MFIter mfi(ldata_p->state, TilingIfNotGPU()); mfi.isValid();
            ...
            for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
              ...
              amrex::ParallelFor(
                ebx, NUM_SPECIES,
                [flux_spec, flux_wbar] AMREX_GPU_DEVICE(
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
          ...
          for (MFIter mfi(ldata_p->state, TilingIfNotGPU()); mfi.isValid();
            ...
            for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
              ...
              amrex::ParallelFor(
                ebx, NUM_SPECIES,
                [flux_spec, flux_soret] AMREX_GPU_DEVICE(
                  int i, int j, int k, int n) noexcept {
      ...
      getDiffusionOp()->avgDownFluxes(
        GetVecOfArrOfPtrs(fluxes), 0, NUM_SPECIES);

      // Compute diffusion term D^{np1,kp1} (or Dhat)
      fluxDivergence(
        GetVecOfPtrs(diffData->Dhat), 0, GetVecOfArrOfPtrs(fluxes), 0,
        NUM_SPECIES, 1, -1.0);
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
      ...
      for (int dTiter = 0; dTiter < m_deltaTIterMax &&
        ...
        if (m_isothermalEB) {
          ...
          getDiffusionOp()->diffuse_scalar(
            GetVecOfPtrs(getTempVect(AmrNewTime)), 0,
            GetVecOfConstPtrs(EBvalue), 0, GetVecOfConstPtrs(rhs), 0,
        ...
        if (
          (dTiter == m_deltaTIterMax - 1) &&
          (deltaT_norm > m_deltaT_norm_max)) {
    ...
    void PeleLM::deltaTIter_prepare(
      const Vector<MultiFab*>& a_rhs, const Vector<MultiFab*>& a_Tsave,
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
        ...
        for (MFIter mfi(ldataNew_p->state, TilingIfNotGPU()); mfi.isValid();
          ...
          auto const& fourier =
            diffData->Dhat[lev].const_array(mfi, NUM_SPECIES);
          auto const& diffDiff =
          ...
          amrex::ParallelFor(
            bx, [=, dt = m_dt] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
              // Assemble deltaT RHS
              rhs(i, j, k) =
                dt * ((rhoH_o(i, j, k) - rhoH_n(i, j, k)) * dtinv +
    ...
    void PeleLM::deltaTIter_update(
      int a_dtiter, const Vector<Array<MultiFab*, AMREX_SPACEDIM>>& a_fluxes,
      const Vector<MultiFab*>& a_ebfluxes,
      const Vector<MultiFab const*>& a_Tsave,
      std::unique_ptr<AdvanceDiffData>& diffData, Real& a_deltaT_norm)
    {
#ifndef AMREX_USE_EB
      amrex::ignore_unused(a_ebfluxes);
#endif
      ...
      if (m_deltaT_verbose) {
        Print() << "   DeltaT solve norm [" << a_dtiter
                << "] = " << a_deltaT_norm << "\n";
      }
      ...
      if (m_isothermalEB) {
        ...
        fluxDivergence(
          GetVecOfPtrs(diffData->Dhat), NUM_SPECIES + 1, a_fluxes,
          NUM_SPECIES + 1, 1, 1, -1.0);
      } else
#endif
      {
        fluxDivergence(
          GetVecOfPtrs(diffData->Dhat), NUM_SPECIES, a_fluxes, NUM_SPECIES, 2,
          1, -1.0);
      }
    ...
    void PeleLM::getScalarDiffForce(
      std::unique_ptr<AdvanceAdvData> & advData,
      std::unique_ptr<AdvanceDiffData> & diffData)
    {
      for (int lev = 0; lev <= finest_level; ++lev) {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
      ...
      fillpatch_forces(
        m_cur_time, GetVecOfPtrs(advData->Forcing),
        advData->Forcing[0].nGrow());
        }
      }

      void PeleLM::computeDivTau(
        const TimeStamp& a_time, const Vector<MultiFab*>& a_divtau,
        int use_density, Real scale)
      {
      ...
      Array<LinOpBCType, AMREX_SPACEDIM> PeleLM::getDiffusionLinOpBC(
        Orientation::Side a_side, const BCRec& a_bc)
      {
      ...
      Vector<Array<LinOpBCType, AMREX_SPACEDIM>> PeleLM::getDiffusionTensorOpBC(
