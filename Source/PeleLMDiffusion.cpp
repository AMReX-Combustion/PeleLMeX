                                               ...
                                               int is_init)
{
   ...
   if (m_isothermalEB) {
      for (int lev = 0; lev <= finest_level; ++lev) {
         EBfluxes[lev].define(grids[lev], dmap[lev], NUM_SPECIES+2, nGrow, MFInfo(), Factory(lev));
      }
   }
#endif
   ...
   if (m_isothermalEB) {
      if (is_init || !m_use_wbar) {
         computeDifferentialDiffusionFluxes(a_time, GetVecOfArrOfPtrs(fluxes), GetVecOfPtrs(EBfluxes), {});
      } else {
         computeDifferentialDiffusionFluxes(a_time, GetVecOfArrOfPtrs(fluxes), GetVecOfPtrs(EBfluxes), GetVecOfArrOfPtrs(diffData->wbar_fluxes));
      }
   } else
#endif
   {
   ...
   if (a_time == AmrOldTime) {
      ...
      if (m_isothermalEB) {
         fluxDivergenceRD(GetVecOfConstPtrs(getTempVect(AmrOldTime)), 0,
                          ...
                          GetVecOfPtrs(EBfluxes), NUM_SPECIES,
...
#else
      fluxDivergence(GetVecOfPtrs(diffData->Dn), 0, GetVecOfArrOfPtrs(fluxes), 0,
      ...
      fluxDivergenceRD(GetVecOfConstPtrs(getRhoHVect(AmrNewTime)), 0,
                       ...
                       1, intensiveFluxes, bcRecRhoH_d.dataPtr(), -1.0, m_dt);
#else
      fluxDivergence(GetVecOfPtrs(diffData->Dnp1), 0, GetVecOfArrOfPtrs(fluxes), 0,
                     NUM_SPECIES+2, intensiveFluxes, -1.0);
#endif
   }
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
   ...
   if (m_isothermalEB) {
      ...
      fluxDivergence(GetVecOfPtrs(diffData->Dhat), NUM_SPECIES, GetVecOfArrOfPtrs(fluxes), NUM_SPECIES,
                     GetVecOfPtrs(EBfluxes), NUM_SPECIES, 1, 1, -1.0);
                               ...
                               Real &a_deltaT_norm)
{
   ...
   if (m_isothermalEB) {
      ...
      fluxDivergence(GetVecOfPtrs(diffData->Dhat), NUM_SPECIES, a_fluxes, NUM_SPECIES,
                     a_ebfluxes, NUM_SPECIES, 1, 1, -1.0);
