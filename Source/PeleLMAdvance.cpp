...
void PeleLM::Advance(int is_initIter) {
	...
	diffData.reset(new AdvanceDiffData(finest_level, grids, dmap, m_factory,
					   m_nGrowAdv, m_use_wbar,
					   m_use_soret));
	std::unique_ptr<AdvanceAdvData> advData;
	advData.reset(new AdvanceAdvData(finest_level, grids, dmap, m_factory,
					 m_incompressible, m_nGrowAdv,
					 m_nGrowMAC));
	...
	// \pi^{n-1/2} + dt/rho^{n+1/2} * F^{n+1/2}
	updateVelocity(advData);

	// Semi-implicit CN diffusion solve to get U^{np1*}
	diffuseVelocity();

	// Nodal projection to get constrained U^{np1} and new pressure
	// \pi^{n+1/2}
	...
	if (m_verbose > 1) {
		...
		amrex::Print() << "   - Advance()::VelocityAdvance  --> Time: "
			       << VelAdvEnd << "\n";
	}
	...
	if (m_verbose > 0) {
		...
		amrex::Print()
		    << " >> PeleLM::Advance() --> Time: " << run_time << "\n";
	}
}

void PeleLM::oneSDC(int sdcIter, std::unique_ptr<AdvanceAdvData> &advData,
		    std::unique_ptr<AdvanceDiffData> &diffData) {
	...
	if (sdcIter > 1) {
		...
		if (m_verbose > 1) {
			Real UpdateEnd =
			    ParallelDescriptor::second() - UpdateStart;
			ParallelDescriptor::ReduceRealMax(
			    UpdateEnd, ParallelDescriptor::IOProcessorNumber());
			amrex::Print()
			    << "   - oneSDC()::Update t^{n+1,k}  --> Time: "
			    << UpdateEnd << "\n";
		}
	}
	//----------------------------------------------------------------
	...
	if (m_verbose > 1) {
		...
		ParallelDescriptor::ReduceRealMax(
		    MACEnd, ParallelDescriptor::IOProcessorNumber());
		amrex::Print()
		    << "   - oneSDC()::MACProjection()   --> Time: " << MACEnd
		    << "\n";
	}
	checkMemory("MAC-Proj");
	BL_PROFILE_VAR_STOP(PLM_MAC);
