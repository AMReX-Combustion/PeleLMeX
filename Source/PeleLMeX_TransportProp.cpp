...
PeleLM::calcDiffusivity(const TimeStamp& a_time)
{
  ...
  for (int lev = 0; lev <= finest_level; ++lev) {
    ...
    const int soret_idx = do_soret ? 1 : 0; //pass soret array, or pass mu as dummy (won't do anything)    
    amrex::ParallelFor(ldata_p->diff_cc, ldata_p->diff_cc.nGrowVect(),
      [=] AMREX_GPU_DEVICE(int box_no, int i, int j, int k) noexcept {
	getTransportCoeff(i,j,k, do_fixed_Le, do_fixed_Pr, do_soret, Le_inv, Pr_inv,
