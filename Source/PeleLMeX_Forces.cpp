...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
  ...
  for (amrex::MFIter mfi(*a_velForce, amrex::TilingIfNotGPU()); mfi.isValid();
    ...
    if (m_do_turbulent_forcing) {
      // note: if m_incompressible == false then m_rho is unused by
      // addTurbVelForces
  ...
  void PeleLM::addSpark(const TimeStamp a_timestamp)
  {
    for (int lev = 0; lev <= finest_level; ++lev) {
  ...
  void PeleLM::addScalarVarianceSources(const TimeStamp a_timestamp)
  {
        ...
        for (int lev = 0; lev <= finest_level; ++lev) {
          ...
          for (int n = 0; n < MANIFOLD_DIM; ++n) {
            if (leosparm.is_variance_of[n] >= 0) {
              ...
              amrex::ParallelFor(
                *m_extSource[lev], [extma, statema, n, C_chi, ScInv,
                                    inv_l_scale2, mut_arr_x, gx, mut_arr_y, gy
#if (AMREX_SPACEDIM == 3)
                                    ,
                                    mut_arr_z, gz
#endif
                  ...
                  const amrex::Real mu_grad2 =
                    fact * (AMREX_D_TERM(
                             mut_arr_x[bx](i, j, k) * gx[bx](i, j, k) *
                                 gx[bx](i, j, k) +
                               mut_arr_x[bx](i + 1, j, k) *
                                 gx[bx](i + 1, j, k) * gx[bx](i + 1, j, k),
                             +mut_arr_y[bx](i, j, k) * gy[bx](i, j, k) *
                                 gy[bx](i, j, k) +
                               mut_arr_y[bx](i, j + 1, k) *
                                 gy[bx](i, j + 1, k) * gy[bx](i, j + 1, k),
                             +mut_arr_z[bx](i, j, k) * gz[bx](i, j, k) *
                                 gz[bx](i, j, k) +
                               mut_arr_z[bx](i, j, k + 1) *
                                 gz[bx](i, j, k + 1) * gz[bx](i, j, k + 1)));
  ...
  void PeleLM::getExternalSources(
    const int is_initIter, const PeleLM::TimeStamp a_timestamp_old,
